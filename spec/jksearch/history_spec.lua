describe("jksearch.history", function()
  ...
  it("returns empty results when the file does not exist", function()
    ...
    assert.is_nil(history.get("実験"))
  end)

  it("stores and loads search results", function()
    local results = {
      {
        title = "実験",
        dict = "日本大百科全書",
    ...
    history.store_search("実験", results)

    local entry = history.get("実験")
  ...
  it("does not overwrite an existing entry", function()
    history.store_search("実験", { { title = "実験", url = "https://example.com/1" } })
    history.store_search("実験", { { title = "別物", url = "https://example.com/9" } })
  ...
  it("stores and gets full text by url", function()
    history.store_search("実験", { { title = "実験", url = "https://example.com/1" } })
    assert.is_nil(history.get_text("実験", "https://example.com/1"))

    history.store_text("実験", "https://example.com/1", "意味の全文")
    assert.are.equal("意味の全文", history.get_text("実験", "https://example.com/1"))

    -- 保存済みの全文は load() の結果にも含まれる
    local entry = history.get("実験")
    assert.are.equal("意味の全文", entry.results[1].text)
  end)

  it("survives save/load roundtrip", function()
    history.store_search("実験", { { title = "実験", url = "https://example.com/1" } })
    history.store_text("実験", "https://example.com/1", "全文")

    -- ファイルから読み直しても同じ内容が得られる
    local data = history.load()
    assert.is_not_nil(data["実験"])
    assert.are.equal("全文", data["実験"].results[1].text)
  end)
end)
